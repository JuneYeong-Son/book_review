...
## 3-3. 추천 시스템 (구현됨) — 코드는 `server/src/recommendation/` 폴더
- **방법 선택** (메인 '추천하는 책' 섹션에서 토글):
  - `content` — **읽었던 책과 비슷한 책**: genre·category·author feature 유사도.
    고도화: 별점 가중(높게 평가한 책 취향 강화) + 최근 읽은 책 recency 가중 + 관심 책 강한 신호 + 인기도 결합.
  - `popular` — **요즘 많이 사는 책**: 알라딘 베스트셀러 API(ItemList). 우리 DB에 있으면 바로 기록/관심,
    없으면 '내 서재에 추가'(임포트) 가능.
- 파일: `recommendation/{types,content_based,popularity_based,index}.ts`.
- 엔드포인트: `GET /books/recommendations?method=content|popular` (선택적 로그인).
- 향후: 협업 필터링·임베딩 기반으로 확장(§4).
- 알라딘 표지: 시드가 실행 시 알라딘에서 각 책의 실제 표지·장르를 가져옴(키 없으면 폴백).

## 3-4. UI 추가 (구현됨)
- 메인 서평/토론: **정사각 카드 + 가로 캐러셀(‹ › 버튼)**. 관심 책/좋아요·댓글 순 정렬.
- 메인: 내가 쓴 **최신 인상깊은 글귀 2개** 표시. 섹션 제목 확대.
- '추천하는 책' 섹션(아이콘 없음).
- 마이페이지 **달력 탭**: 그날 무슨 책을 얼마나 읽었는지(분량) 간략 표시.
- 토론 시드 추가(책벌레 bookworm/password 계정 포함).

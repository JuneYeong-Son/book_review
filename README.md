...
- **공지사항**: 관리자가 작성·고정·삭제(`/admin`), 홈 상단 배너 + `/notices` 목록으로 노출.
- **피드백/신고/관리자**: 푸터 '의견·버그 신고'. 관리자(`/admin`) 대시보드 + **회원 관리(권한·활동 정지·삭제)** + 피드백 조회 + 공지 관리 + 신고 게시물 삭제.
- **문서**: 이용 약관(`/terms`) · 개인정보 처리방침(`/privacy`).
...
`docs/progress/todo.md` 참고 — 앱 서명 AAB 빌드/스토어 제출, 앱 소셜 로그인(딥링크), iOS 앱, 추천 고도화 등.
리팩터 이력은 [`docs/architecture/log.md`](docs/architecture/log.md).
